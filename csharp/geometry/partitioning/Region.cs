...
namespace org.apache.commons.math3.geometry.partitioning
{
	...
	/// is composed of several <seealso cref="SubHyperplane sub-hyperplanes"/>,
	...
	public interface Region<S> where S : org.apache.commons.math3.geometry.Space
	{
		...
		/// <summary>
		/// Enumerate for the location of a point with respect to the region. </summary>
		...
		/// <param name="newTree"> inside/outside BSP tree representing the new region </param>
		...
		/// attributes and immutable objects).</p> </summary>
		/// <returns> a new region, copy of the instance </returns>
		Region<S> copySelf();
		...
		/// <summary>
		/// Check if the sub-tree starting at a given node is empty. </summary>
		/// <param name="node"> root node of the sub-tree (<em>must</em> have {@link
		/// Region Region} tree semantics, i.e. the leaf nodes must have
		/// {@code Boolean} attributes representing an inside/outside
		/// property) </param>
		/// <returns> true if the sub-tree starting at the given node is empty </returns>
//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
//ORIGINAL LINE: boolean isEmpty(final BSPTree<S> node);
		bool isEmpty(BSPTree<S> node);

		/// <summary>
		/// Check if the instance covers the full space. </summary>
		/// <returns> true if the instance covers the full space </returns>
		bool Full {get;}

		/// <summary>
		/// Check if the sub-tree starting at a given node covers the full space. </summary>
		/// <param name="node"> root node of the sub-tree (<em>must</em> have {@link
		/// Region Region} tree semantics, i.e. the leaf nodes must have
		/// {@code Boolean} attributes representing an inside/outside
		/// property) </param>
		/// <returns> true if the sub-tree starting at the given node covers the full space </returns>
//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
//ORIGINAL LINE: boolean isFull(final BSPTree<S> node);
		bool isFull(BSPTree<S> node);

		/// <summary>
		/// Check if the instance entirely contains another region. </summary>
		/// <param name="region"> region to check against the instance </param>
		/// <returns> true if the instance contains the specified tree </returns>
//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
//ORIGINAL LINE: boolean contains(final Region<S> region);
...
//ORIGINAL LINE: Region_Location checkPoint(final org.apache.commons.math3.geometry.Point<S> point);
		...
		/// <summary>
		/// Project a point on the boundary of the region. </summary>
		/// <param name="point"> point to check </param>
...
//ORIGINAL LINE: BoundaryProjection<S> projectToBoundary(final org.apache.commons.math3.geometry.Point<S> point);
		...
		/// <seealso cref="BoundaryAttribute BoundaryAttribute"/> objects representing
		...
		/// </summary>
		/// <param name="includeBoundaryAttributes"> if true, the boundary attributes
		...
		/// <returns> underlying BSP tree </returns>
		/// <seealso cref= BoundaryAttribute </seealso>
//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
//ORIGINAL LINE: BSPTree<S> getTree(final boolean includeBoundaryAttributes);
		BSPTree<S> getTree(bool includeBoundaryAttributes);

		/// <summary>
		/// Get the size of the boundary. </summary>
		/// <returns> the size of the boundary (this is 0 in 1D, a length in
		/// 2D, an area in 3D ...) </returns>
		double BoundarySize {get;}
		...
		/// <summary>
		/// Get the barycenter of the instance. </summary>
		/// <returns> an object representing the barycenter </returns>
		Point<S> Barycenter {get;}

		/// <summary>
		/// Compute the relative position of the instance with respect to an
		/// hyperplane. </summary>
		/// <param name="hyperplane"> reference hyperplane </param>
		/// <returns> one of <seealso cref="Side#PLUS Side.PLUS"/>, {@link Side#MINUS
		/// Side.MINUS}, <seealso cref="Side#BOTH Side.BOTH"/> or {@link Side#HYPER
		/// Side.HYPER} (the latter result can occur only if the tree
		/// contains only one cut hyperplane) </returns>
//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
//ORIGINAL LINE: Side side(final Hyperplane<S> hyperplane);
		Side side(Hyperplane<S> hyperplane);

		/// <summary>
		/// Get the parts of a sub-hyperplane that are contained in the region.
		/// <p>The parts of the sub-hyperplane that belong to the boundary are
		/// <em>not</em> included in the resulting parts.</p> </summary>
		/// <param name="sub"> sub-hyperplane traversing the region </param>
		/// <returns> filtered sub-hyperplane </returns>
//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
//ORIGINAL LINE: SubHyperplane<S> intersection(final SubHyperplane<S> sub);
		SubHyperplane<S> intersection(SubHyperplane<S> sub);

	}

	public enum Region_Location
	{
		/// <summary>
		/// Code for points inside the partition. </summary>
		INSIDE,

		/// <summary>
		/// Code for points outside of the partition. </summary>
		OUTSIDE,

		/// <summary>
		/// Code for points on the partition boundary. </summary>
		BOUNDARY
	}
