...
namespace org.apache.commons.math3.optimization.fitting
{
	...
	/// based on integrals.
	/// 
	/// @version $Id: HarmonicFitter.java 1591835 2014-05-02 09:04:01Z tn $ </summary>
...
//ORIGINAL LINE: public HarmonicFitter(final org.apache.commons.math3.optimization.DifferentiableMultivariateVectorOptimizer optimizer)
		public HarmonicFitter(DifferentiableMultivariateVectorOptimizer optimizer) : base(optimizer)
		{
		}
		...
		/// </summary>
		/// <returns> the parameters of the harmonic function that best fits the
		/// observed points (see the other <seealso cref="#fit(double[]) fit"/> method. </returns>
		/// <exception cref="NumberIsTooSmallException"> if the sample is too short for the
		/// the first guess to be computed. </exception>
		/// <exception cref="ZeroException"> if the first guess cannot be computed because
		/// the abscissa range is zero. </exception>
		public virtual double[] fit()
		{
			return fit((new ParameterGuesser(Observations)).guess());
		}
...
//ORIGINAL LINE: final WeightedObservedPoint[] observations = unsorted.clone();
			...
			/// This method assumes that the <seealso cref="#sortObservations(WeightedObservedPoint[])"/> method
