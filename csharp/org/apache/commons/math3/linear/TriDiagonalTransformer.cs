...
//ORIGINAL LINE: final int m = matrix.getRowDimension();
		...
		/// <p>Q is an orthogonal matrix, i.e. its transpose is also its inverse.</p> </summary>
...
//ORIGINAL LINE: double[][] qta = new double[m][m];
					...
					{
	//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
	//ORIGINAL LINE: final double[] hK = householderVectors[k - 1];
						double[] hK = householderVectors[k - 1];
						qta[k][k] = 1;
						if (hK[k] != 0.0)
						{
	//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
	//ORIGINAL LINE: final double inv = 1.0 / (secondary[k - 1] * hK[k]);
		...
		/// <summary>
		/// Returns the tridiagonal matrix T of the transform. </summary>
		...
		public virtual RealMatrix T
		{
			get
			{
				...
				{
	//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
	//ORIGINAL LINE: final int m = main.length;
					int m = main.Length;
//JAVA TO C# CONVERTER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
//ORIGINAL LINE: double[][] ta = new double[m][m];
					double[][] ta = RectangularArrays.ReturnRectangularDoubleArray(m, m);
					for (int i = 0; i < m; ++i)
					{
						ta[i][i] = main[i];
						if (i > 0)
...
//ORIGINAL LINE: final int m = householderVectors.length;
			int m = householderVectors.Length;
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final double[] z = new double[m];
...
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final double a = (hK[k + 1] > 0) ? -org.apache.commons.math3.util.FastMath.sqrt(xNormSqr) : org.apache.commons.math3.util.FastMath.sqrt(xNormSqr);
				...
				if (a != 0.0)
				{
					...
					hK[k + 1] -= a;
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final double beta = -1 / (a * hK[k + 1]);
...
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final double hKI = hK[i];
						double hKI = hK[i];
						double zI = hI[i] * hKI;
						for (int j = i + 1; j < m; ++j)
						{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final double hIJ = hI[j];
							double hIJ = hI[j];
							zI += hIJ * hK[j];
							z[j] += hIJ * hKI;
						}
