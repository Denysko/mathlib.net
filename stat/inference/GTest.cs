...
namespace mathlib.stat.inference
{
	...
	/// 
	/// @version $Id: GTest.java 1547633 2013-12-03 23:03:06Z tn $
	/// @since 3.1
	/// </summary>
	public class GTest
	{
		...
		/// <returns> G-Test statistic </returns>
		/// <exception cref="NotPositiveException"> if {@code observed} has negative entries </exception>
		/// <exception cref="NotStrictlyPositiveException"> if {@code expected} has entries that
...
//ORIGINAL LINE: final double dev = rescale ? mathlib.util.FastMath.log((double) observed[i] / (ratio * expected[i])) : mathlib.util.FastMath.log((double) observed[i] / expected[i]);
		...
		/// <seealso cref="#g(double[], long[]) g(expected, observed)"/>
...
//ORIGINAL LINE: final mathlib.distribution.ChiSquaredDistribution distribution = new mathlib.distribution.ChiSquaredDistribution(expected.length - 1.0);
		...
		/// <returns> p-value </returns>
		/// <exception cref="NotPositiveException"> if {@code observed} has negative entries </exception>
		/// <exception cref="NotStrictlyPositiveException"> {@code expected} has entries that are
...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
		...
		/// to form a vector. See <seealso cref="#entropy(long[])"/>.
		/// </summary>
		/// <param name="k"> 2 Dimensional Matrix of long values (for ex. the counts of a
		/// trials) </param>
		/// <returns> Shannon Entropy of the given Matrix
		///  </returns>
//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
//ORIGINAL LINE: private double entropy(final long[][] k)
		private double entropy(long[][] k)
		{
			...
			for (int i = 0; i < k.Length; i++)
			{
				for (int j = 0; j < k[i].Length; j++)
				{
					...
					{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final double p_ij = (double) k[i][j] / sum_k;
		...
		/// </summary>
		/// <param name="k"> Vector (for ex. Row Sums of a trials) </param>
...
//ORIGINAL LINE: private double entropy(final long[] k)
		private double entropy(long[] k)
		{
			...
			for (int i = 0; i < k.Length; i++)
			{
				...
				{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final double p_i = (double) k[i] / sum_k;
		...
		/// <param name="observed2"> array of observed frequency counts of the second data
		/// set </param>
		/// <returns> G-Test statistic </returns>
		/// <exception cref="DimensionMismatchException"> the the lengths of the arrays do not
		/// match or their common length is less than 2 </exception>
		/// <exception cref="NotPositiveException"> if any entry in {@code observed1} or
		/// {@code observed2} is negative </exception>
		/// <exception cref="ZeroException"> if either all counts of
...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
		public virtual double gDataSetsComparison(long[] observed1, long[] observed2)
		{
			...
			// Compute and compare count sums
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final long[] collSums = new long[observed1.length];
			long[] collSums = new long[observed1.Length];
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final long[][] k = new long[2][observed1.length];
//JAVA TO C# CONVERTER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
//ORIGINAL LINE: long[][] k = new long[2][observed1.Length];
			long[][] k = RectangularArrays.ReturnRectangularLongArray(2, observed1.Length);
...
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final long[] rowSums = {countSum1, countSum2};
			long[] rowSums = new long[] {countSum1, countSum2};
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final double sum = (double) countSum1 + (double) countSum2;
		...
		/// <seealso cref="#gDataSetsComparison(long[], long[] )"/>.
		/// 
		...
		/// </summary>
		/// <param name="k11"> number of times the two events occurred together (AB) </param>
		/// <param name="k12"> number of times the second event occurred WITHOUT the
		/// first event (notA,B) </param>
		/// <param name="k21"> number of times the first event occurred WITHOUT the
		/// second event (A, notB) </param>
		/// <param name="k22"> number of times something else occurred (i.e. was neither
		/// of these events (notA, notB) </param>
		/// <returns> root log-likelihood ratio
		///  </returns>
//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
//ORIGINAL LINE: public double rootLogLikelihoodRatio(final long k11, long k12, final long k21, final long k22)
		public virtual double rootLogLikelihoodRatio(long k11, long k12, long k21, long k22)
		{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final double llr = gDataSetsComparison(new long[]{k11, k12}, new long[]{k21, k22});
		...
		/// same distribution. </p>
		/// 
		/// <p>See <seealso cref="#gTest(double[], long[])"/> for details
		...
		/// set </param>
		/// <returns> p-value </returns>
		/// <exception cref="DimensionMismatchException"> the the length of the arrays does not
		/// match or their common length is less than 2 </exception>
		/// <exception cref="NotPositiveException"> if any of the entries in {@code observed1} or
		/// {@code observed2} are negative </exception>
		/// <exception cref="ZeroException"> if either all counts of {@code observed1} or
		/// {@code observed2} are zero, or if the count at some index is
		/// zero for both arrays </exception>
		/// <exception cref="MaxCountExceededException"> if an error occurs computing the
		/// p-value. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
...
//ORIGINAL LINE: final mathlib.distribution.ChiSquaredDistribution distribution = new mathlib.distribution.ChiSquaredDistribution((double) observed1.length - 1);
		...
		/// </p>
		/// <p>See <seealso cref="#gDataSetsComparison(long[], long[])"/> for details
		/// on the formula used to compute the G (LLR) statistic used in the test and
		/// <seealso cref="#gTest(double[], long[])"/> for information on how
		...
		/// {@code observed2} are zero, or if the count at some index is
		/// zero for both arrays </exception>
		/// <exception cref="OutOfRangeException"> if {@code alpha} is not in the range
		/// (0, 0.5] </exception>
