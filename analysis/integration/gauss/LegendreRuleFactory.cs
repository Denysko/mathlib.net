...
namespace mathlib.analysis.integration.gauss
{
    ...
    public class LegendreRuleFactory : BaseRuleFactory<double?>
    {
        ...
        protected internal override Pair<double?[], Double[]> computeRule(int numberOfPoints)
        {
            ...
            //ORIGINAL LINE: final Double[] previousPoints = getRuleInternal(numberOfPoints - 1).getFirst();
            double?[] previousPoints = getRuleInternal(numberOfPoints - 1).First;

            // Compute next rule.
            //JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
            //ORIGINAL LINE: final Double[] points = new Double[numberOfPoints];
            double?[] points = new double?[numberOfPoints];
            //JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
            //ORIGINAL LINE: final Double[] weights = new Double[numberOfPoints];
